...
//        return json_encode(1);
    ...
    public function listarServicios_destino(Request $request)
    {
        ...
        $costos=M_Producto::get();
        $categoria_id = $id;
        return view('admin.contabilidad.lista-servicios',compact(['id','destino','destinations','sericios','proveedores','costos','categoria_id']));
    }
...
//        return $sericios;
        ...
        foreach ($sericios->where('ruta_salida',$ruta[0])->where('ruta_llegada',$ruta[1]) as $servicio) {
            $cadena .= '<tr class="' . $servicio->localizacion . '" id="lista_services_' . $servicio->id . '">
                            <td class="text-green-goto">' . $servicio->codigo . '</td>';
            if($destino[1] == 'MOVILID'){
                $cadena .= '<td>'.$servicio->ruta_salida.'-'.$servicio->ruta_llegada.'</td>';
            }
            $cadena.='<td id="tipo_' . $servicio->id . '">' . $servicio->tipoServicio;
            if ($destino[1] == 'MOVILID')
                $cadena .= '[' . $servicio->min_personas . ' - ' . $servicio->max_personas . ']';
            $cadena .= '</td>
    <td id="nombre_' . $servicio->id . '">' . $servicio->nombre . '</td>';
